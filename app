...
if [ "$1" = "deploy" ]
then
  echo "Build containers for production using cache"
  docker-compose -f docker-compose.yml build --no-cache
fi

if [ "$1" = "force-deploy" ]
then
  echo "Build containers for production without using cache"
  docker-compose -f docker-compose.yml build --no-cache
fi

if [ "$1" = "force-build" ]
then
  echo "Build containers without using cache"
...
if [ "$1" = "prune" ]
then
  echo "Remove containers, images, cache etc."
  docker system prune
fi
